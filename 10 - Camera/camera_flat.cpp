...
void triangle(Vec3f t[3], float* zbuffer, TGAImage &image, TGAImage &texture, float intensity) {
...
Matrix4x4 lookat(Vec3f eye, Vec3f center, Vec3f up)
{
...
int main(int argc, char** argv) {
    ...
    Matrix4x4 view = lookat(eye,center,up);
    ...
    for (int i = 0; i < model.nfaces(); i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            screen[j] = xyz(viewport * projection * view * xyzw(world[j]));
        }
       ...
       triangle(screen, zbuffer, image, texture, intensity);
    }

    image.flip_vertically();
    string filename = "camera_flat.tga";
